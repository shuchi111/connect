        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {blogs.map((b, i) => (
            <motion.a
              ...
              data-testid={`blog-${i}`}
              className="group bg-zinc-950 border border-white/10 p-6 hover:border-white/30 transition-all cursor-pointer block"
            >
              <div className="flex items-center justify-between text-[11px] font-mono uppercase tracking-widest">
                ...
                <span className="inline-flex items-center gap-1 text-white/40">
                  <SourceIcon source={b.source} /> {b.readTime}
                </span>
              </div>
              ...
              <div className="mt-5 inline-flex items-center gap-1.5 text-xs text-white/50 group-hover:text-white">
                Read on {b.source} <ArrowUpRight size={12} />
              </div>
            </motion.a>
          ))}
        </div>
